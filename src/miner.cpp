...
// Copyright (c) 2013 The KateCoin developers
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake, int64_t* pFees)
{
    ...
    int64_t nFees = 0;
    {
        ...
        if (!fProofOfStake)
        {
            ...
            // If creating a legacy block, set the tx timestamps for compatibility between 1.5 and 2.0 nodes.
...
void StakeMiner(CWallet *pwallet)
{
    ...
    while (true)
    {
        ...
        // Special restriction for KateCoin to not allow staking while still in PoW mode.
        while (pwallet->IsLocked() || !pwallet->IsCrypted() || nBestHeight < LAST_POW_BLOCK)
        {
            MilliSleep(1000);
            if (fShutdown)
                return;

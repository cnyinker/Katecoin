...
#ifdef QT_GUI
const char *walletUrl = "http://katecoin.com/";
const char *walletDownloadsUrl = "http://blockexplorer.katecoin.com/downloads/";
const char *claimsUrl = "http://www.katecoin.com";
const char *chatUrl = "http://katecoin.com/en/chat/";
const char *blockchainUrl = "http://blockexplorer.katecoin.com/";
bool fRestart = false;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KateCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KateCoin
    // Mac: ~/Library/Application Support/KateCoin
    // Unix: ~/.katecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KateCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "katecoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong KateCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("KateCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {

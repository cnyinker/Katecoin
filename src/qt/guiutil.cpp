...
void setupAmountWidget(QLineEdit *widget, QWidget *parent)
{
    ...
    amountValidator->setDecimals(BitcoinUnits::decimals(BitcoinUnits::KTC));
...
{
    // KateCoin: check prefix
    if(uri.scheme() != QString("katecoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::KTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert katecoin:// to katecoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("katecoin://"))
    {
        uri.replace(0, 12, "katecoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KateCoin.lnk";
}
...
{
    return GetAutostartDir() / "katecoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("KateCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  katecoin-qt [" + tr("command-line options") + "]                     " + "\n";

...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non KTC unit input in lnReqAmount, we generate the URI with KTC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::formatMaxDecimals(BitcoinUnits::KTC, ui->lnReqAmount->value(), BitcoinUnits::maxdecimals(BitcoinUnits::KTC)));
            paramCount++;
        }
        else
        {

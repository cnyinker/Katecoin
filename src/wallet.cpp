    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s KTC %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s KTC %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
//PoST
bool CWallet::CreateCoinStakeTime(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && !fShutdown && pindexPrev == pindexBest; n++)
        {
            ...
            if (CheckStakeTimeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake, targetProofOfStake, pindexPrev))
            {
                ...
                if (fDebug && GetBoolArg("-printcoinstake"))
                    printf("CreateCoinStakeTime : kernel found\n");
                ...
                {
                    if (fDebug && GetBoolArg("-printcoinstake"))
                        printf("CreateCoinStakeTime : failed to parse kernel\n");
                    break;
                }
                if (fDebug && GetBoolArg("-printcoinstake"))
                    printf("CreateCoinStakeTime : parsed kernel type=%d\n", whichType);
                if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH)
                {
                    if (fDebug && GetBoolArg("-printcoinstake"))
                        printf("CreateCoinStakeTime : no support for kernel type=%d\n", whichType);
                ...
                if (whichType == TX_PUBKEYHASH) // pay to address type
                {
                    ...
                    {
                        if (fDebug && GetBoolArg("-printcoinstake"))
                            printf("CreateCoinStakeTime : failed to get key for kernel type=%d\n", whichType);
                ...
                {
                    if (fDebug && GetBoolArg("-printcoinstake"))
                        printf("CreateCoinStakeTime : invalid key for kernel type=%d\n", whichType);
                        break; // keys mismatch
                    }
                ...
                if (fDebug && GetBoolArg("-printcoinstake"))
                    printf("CreateCoinStakeTime : added kernel type=%d\n", whichType);
                fKernelFound = true;
                break;
            }
        }
    ...
    // Calculate stake time reward
    {
        ...
        if (!txNew.GetStakeTime(txdb, nStakeTime, pindexPrev))
            return error("CreateCoinStakeTime : failed to calculate coin age");
    ...
    BOOST_FOREACH(const CWalletTx* pcoin, vwtxPrev)
    {
        if (!SignSignature(*this, *pcoin, txNew, nIn++))
            return error("CreateCoinStakeTime : failed to sign coinstake");
    }

    // Limit size
    unsigned int nBytes = ::GetSerializeSize(txNew, SER_NETWORK, PROTOCOL_VERSION);
    if (nBytes >= MAX_BLOCK_SIZE_GEN/5)
        return error("CreateCoinStakeTime : exceeded coinstake size limit");
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s KTC %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s KTC %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
